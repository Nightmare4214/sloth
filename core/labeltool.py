...
class LabelTool(QObject):
    ...
    def saveAnnotations(self, fname, test_flag=False, pre=-1, all_flag=False):
        ...
        try:
            ...
            if fname is None:
                self._container = self._container_factory.create('temp.json')
            else:
                self._container = self._container_factory.create(fname)
            ...
            if fname is None:
                msg = "Successfully saved %s (%d files, %d annotations)" % \
                      ('test.json', self._model.root().numFiles(), self._model.root().numAnnotations())
            else:
                msg = "Successfully saved %s (%d files, %d annotations)" % \
                      (fname, self._model.root().numFiles(), self._model.root().numAnnotations())
            success = True
...
def getNewFile(ann):
    for annontation in ann:
        ...
        filename = annontation['filename']
        directory = os.path.dirname(os.path.abspath(filename))
        json_name = os.path.splitext(os.path.basename(filename))[0] + '.json'
        # print(json.dump([annontation]))
